...
HRESULT InitResult(void)
{
	...
	//テクスチャの読み込み
	D3DXCreateTextureFromFile(pDevice, "data\\TEXTURE\\bg000.jpg", &g_pTextureResult[0]);
	D3DXCreateTextureFromFile(pDevice, "data\\TEXTURE\\gameclear_logo.png", &g_pTextureResult[1]);

	//頂点バッファの生成
	if (FAILED(pDevice->CreateVertexBuffer(sizeof(VERTEX_2D) * 4 * MAX_POLYGON, D3DUSAGE_WRITEONLY, FVF_VERTEX_2D, D3DPOOL_MANAGED, &g_pVtxBuffResult, NULL)))
	{
		return E_FAIL;
	}
